...
func get_code(base_register: int = 1, depth: int = 0) -> Array[URCLInstruction]:
	...
	var bitand = URCLAnd.new()
	bitand.operands.append(URCLRegister.create(base_register))
	bitand.operands.append(URCLRegister.create(base_register))
	bitand.operands.append(URCLRegister.create(base_register + 1))
